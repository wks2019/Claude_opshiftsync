 ...
 * Creates a draft course under the caller's tenant.
 */
export async function POST(request: Request): Promise<NextResponse> {
  return handle(async () => {
    const { userId } = await requireUser()
    const { supabase, hotelGroupId } = await requireAdmin(userId)
    ...
    if (error || !data) {
      throw new ApiError('INTERNAL_ERROR', `Failed to create course: ${error?.message}`, 500)
    }
